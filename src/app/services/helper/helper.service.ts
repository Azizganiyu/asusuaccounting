...
export class HelperService {
  ...
  setLogo(logo){
    if (isPlatformBrowser(this.platformId)) {
      return window.localStorage.setItem('logo', this.getApiImageLink()+'/logo/'+logo);
    }
  }

  getLogo(){
    if (isPlatformBrowser(this.platformId)) {
      return window.localStorage.hasOwnProperty("logo")? window.localStorage.getItem('logo') : null;
    }
    else{
      return null
    }
